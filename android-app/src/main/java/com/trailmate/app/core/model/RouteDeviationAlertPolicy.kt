...
object RouteDeviationAlertPolicy {
    ...
    fun evaluate(
        ...
        val worsened = lastAlertAt != null &&
            fix.crossTrackErrorMeters - state.lastAlertCrossTrackErrorMeters >= ESCALATION_DELTA_METERS
    ...
    private fun waitForReliableFix(
        ...
        RouteDeviationAlertDecision(
            ...
            title = "等待定位稳定",
            caption = fix?.let {
                "当前定位精度约 ${it.horizontalAccuracyMeters.toInt()} m，先到开阔处等待可靠定位，再判断是否偏离路线。"
